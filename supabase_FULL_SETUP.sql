...
begin
  ...
  v_class := coalesce(new.raw_user_meta_data->>'class_grade', '');

  if v_username !~ '^[a-z0-9_]{3,20}$' or exists (select 1 from public.profiles p where p.username = v_username) then
    v_username := 'user_' || substr(new.id::text, 1, 8);
  end if;

  insert into public.profiles (id, username, display_name, phone, class_grade, xp, level, streak, learned, badges, role, last_visit)
  values (new.id, v_username, v_display_name, v_phone, v_class, 0, 1, 0, '{}', '{}', 'user', null)
  on conflict (id) do nothing;
